        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">تعديل الحقل المخصص: {{ $customField->label }}</h4>
                    ...
                    <form action="{{ route('custom-fields.update', $customField) }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                           id="name" name="name" value="{{ old('name', $customField->name) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('label') is-invalid @enderror" 
                                           id="label" name="label" value="{{ old('label', $customField->label) }}" 
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('field_type') is-invalid @enderror" 
                                        ...
                                        <option value="text" {{ old('field_type', $customField->field_type) == 'text' ? 'selected' : '' }}>{{ __('messages.text') }}</option>
                        <option value="number" {{ old('field_type', $customField->field_type) == 'number' ? 'selected' : '' }}>{{ __('messages.number') }}</option>
                        <option value="date" {{ old('field_type', $customField->field_type) == 'date' ? 'selected' : '' }}>{{ __('messages.date') }}</option>
                        <option value="select" {{ old('field_type', $customField->field_type) == 'select' ? 'selected' : '' }}>{{ __('messages.select') }}</option>
                        <option value="checkbox" {{ old('field_type', $customField->field_type) == 'checkbox' ? 'selected' : '' }}>{{ __('messages.checkbox') }}</option>
                                    </select>
                                    @error('field_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('applies_to') is-invalid @enderror" id="applies_to" name="applies_to" required>
                                        ...
                                        <option value="employee" {{ old('applies_to', $customField->applies_to) == 'employee' ? 'selected' : '' }}>{{ __('messages.employee') }}</option>
                        <option value="branch" {{ old('applies_to', $customField->applies_to) == 'branch' ? 'selected' : '' }}>{{ __('messages.branch') }}</option>
                        <option value="tool" {{ old('applies_to', $customField->applies_to) == 'tool' ? 'selected' : '' }}>{{ __('messages.tool') }}</option>
                        <option value="advance" {{ old('applies_to', $customField->applies_to) == 'advance' ? 'selected' : '' }}>{{ __('messages.advance') }}</option>
                        <option value="document" {{ old('applies_to', $customField->applies_to) == 'document' ? 'selected' : '' }}>{{ __('messages.document') }}</option>
                                    </select>
                                    @error('applies_to')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="mb-3" id="options_container" style="display: none;">
                            ...
                            <textarea class="form-control @error('options') is-invalid @enderror" 
                                      ...
                                      placeholder="اكتب كل خيار في سطر منفصل">{{ old('options', is_array($customField->options) ? implode("\n", $customField->options) : '') }}</textarea>
